...
public class ThemePreview extends Preference {
    ...
    public void setTheme(CharSequence packageName) {
        ...
        themePreview = null;
        if (!packageName.equals(Constants.APOLLO)) {
            ...
            if (themeResources != null) {
                int themeNameId = themeResources.getIdentifier(Constants.THEME_TITLE, "string",
                        packageName.toString());
                if (themeNameId != 0) {
                    themeName = themeResources.getString(themeNameId);
                }
                int themeDescriptionId = themeResources.getIdentifier(Constants.THEME_DESCRIPTION, "string",
                        packageName.toString());
                if (themeDescriptionId != 0) {
                    themeDescription = themeResources.getString(themeDescriptionId);
                }
                int themePreviewId = themeResources.getIdentifier(Constants.THEME_PREVIEW, "drawable",
                        packageName.toString());
                if (themePreviewId != 0) {
